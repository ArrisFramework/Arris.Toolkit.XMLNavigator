...
Prerequisites: 
```bash
sudo apt install php-{xmlreader,mbstring,curl}
# or
sudo apt install php8.3-{xmlreader,mbstring,curl}
```
...
XML;
$result =
    \Arris\Toolkit\XMLNavigator\Convertation\FastXmlToArray
    ::prettyPrint($xml);
...
`composer require karelwintersky/arris.xml-navigator`
...
function parseFirstElement(string $filename): void
{
    ...
    $result =
        \Arris\Toolkit\XMLNavigator\Extraction\PrettyPrintComposer
        ::compose($reader);
...
$extractor = \Arris\Toolkit\XMLNavigator\Parsing\FastXmlParser
    ::extractPrettyPrint(
        $reader,
...
$converter = new \Arris\Toolkit\XMLNavigator\Convertation\XmlConverter(
    \Arris\Toolkit\XMLNavigator\General\Notation::VAL,
    \Arris\Toolkit\XMLNavigator\General\Notation::ATTR,
);
...
$content = \Arris\Toolkit\XMLNavigator\Convertation\FastXmlToArray
::convert($xml);
$navigator = 
new \Arris\Toolkit\XMLNavigator\Navigation\XmlElement($content);
...
foreach ($attributes as $attribute) {
    /** @var \Arris\Toolkit\XMLNavigator\Navigation\IXmlAttribute $attribute */
...
/** @var \Arris\Toolkit\XMLNavigator\Navigation\IXmlElement $elem */
...
foreach ($navigator->pull() as $pulled) {
    /** @var \Arris\Toolkit\XMLNavigator\Navigation\IXmlElement $pulled */
...
/** @var \Arris\Toolkit\XMLNavigator\Navigation\IXmlElement $nested */
...
foreach ($nested->pull('b') as $b) {
    /** @var \Arris\Toolkit\XMLNavigator\Navigation\IXmlElement $b */
